    ...
    class Accept < SlackRubyBot::Commands::Base
      def self.call(client, data, _match)
        ...
        if challenge
          ...
          send_message_with_gif client, data.channel, "#{challenge.challenged.map(&:user_name).join(' and ')} accepted #{challenge.challengers.map(&:user_name).join(' and ')}'s challenge.", 'ping pong'
